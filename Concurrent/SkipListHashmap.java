...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        if (key == null)
            throw new NullPointerException();
        Comparator<? super K> cmp = comparator; //使用了一个比较器
        outer: for (;;) {//cas的外侧while循环,如果插入失败还可以在继续一次
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
              //找到要查入的点b,和b的后一个点n
                if (n != null) { //需要在两个节点中间插入
                    Object v; int c;
                    Node<K,V> f = n.next;//f是n的next,这样我们其实已经取了3个节点.
                    if (n != b.next)//先判断一次n这时候还是不是b的next,否在就直接再找一次b的next               // inconsistent read
                        break;
                    //在删除时会先将value设置为null,所以这样可以立刻看到value为null,所以重新进行操作.
                    if ((v = n.value) == null) {   // n is deleted
                        n.helpDelete(b, f);
                        break;
                    }
                    ...
                    if (c == 0) { //就是说n的key值和要查如的相同,那么就要处理键冲突
                        if (onlyIfAbsent || n.casValue(v, value)) { //需要插入到n中去
                            @SuppressWarnings("unchecked") V vv = (V)v;
                            return vv;
                        }
    ...
    final V doRemove(Object key, Object value) {
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                //到这步时,已经找到了要删除的东西
                if (value != null && !value.equals(v)) //在进行一次判断是否能够找到了要删除的节点.
                    break outer;
                if (!n.casValue(v, null)) //先将值设成空,如果已经将这个节点删除了,那么这个设置为null也是没有任何影响.
                    break;
                //在n和f之间插入一个marker
                if (!n.appendMarker(f) || !b.casNext(n, f)) //将f设置成b的next
