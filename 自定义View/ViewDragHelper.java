...
public class ViewDragHelper {
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {  //如果是DOWN,说明全部设置都被重置，不需要处理click事件
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                saveInitialMotion(x, y, pointerId);//保存最初的位置

                final View toCapture = findTopChildUnder((int) x, (int) y); //找当前位置下的View
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    if (pastSlop) {
                        ...
                        if ((horizontalDragRange == 0 || horizontalDragRange > 0
                            ...
                            break; //就是直接不在进行，也是博文中clickable=true无法移动的原因
                        }
                    }
                    ...
                    // tryCaptureViewForDrag -> captureView - > setDragState
                    if (pastSlop && tryCaptureViewForDrag(toCapture, pointerId)) {
                        break;
                    }
                }
                //如果clampVertical返回的left就是targetTop.如果是以前的不会动了。
                saveLastMotion(ev);
                break;
            }
