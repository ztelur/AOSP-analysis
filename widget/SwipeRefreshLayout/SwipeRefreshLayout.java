...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //这是先让组件自己measure去啦
        if (mTarget == null) {
            ensureTarget(); //mTarget是除了mProgres之外的一个child view
        }
        if (mTarget == null) {
            return;
        }
            ...
            if (mTarget instanceof AbsListView) {
                ...
                return absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0)
                                ...
                                //注意如何判断是否可以滑动啊,如果第一个显示的position不是0,并且第一个child的
                                //getTop要小于paddingTop()那么就说明可以继续向上滑动
            } else {
                //AbsListView和scrollView的实现是不同的，前者scrollY一直都是０
                return mTarget.getScrollY() > 0;
            }
        } else {
            //如果版本大约14那么就直接让view向上移动一个像素就可以啦
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        // 如果正在refreshing或者子view可以向上移动的画,直接返回false,不拦截事件
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //获得mActivePointerId的y值坐标？？？？
                final float y = getMotionEventY(ev, mActivePointerId);
                if (y == -1) {
                    return false;
                }
                final float yDiff = y - mInitialDownY;
                //可以看出，在判断是否拦截的函数中 move中比较当前y和点击下时的y的差距
                //并且当前mIsBeingDragged = false;开始准备滑动
